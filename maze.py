...
class PerfectMaze(object):
    ...
    Parameters:
        ...
        visualize: (bool) step-by-step visualisation of maze generation
    ...
    Methods:
        ...
        get_T_matrix():
            Getter for sparse transition matrix
    ...
    def __init__(self, N, visualize=False):
        ...
        while len(self.__sets) > 1:
            ...
            if s1 != s2:
                ...
                self.__removearray(self.__sets, self.__sets[s2])
            else:
                if dirflag == 0:
                    whc += 1
                else:
        ...
        self.T = sp.csr_matrix((data, (row, col)), shape=(N**2, N**2))
    ...
    def __path_search(self, row, col, start, end, parent):
        if start == end:
            return [start]
        elif len(col[np.where(row == start)[0]]) < 1:
            return -1
        else:
